    ...
    public function index() {
        if (count($_POST) > 0) {
            ...
            if (count($arr_admin_details_with_username) > 0) {
                ...
                $arr_admin_details_username_password = $this->common_model->getRecords("mst_users", "*", array("user_name" => $this->input->post('user_name'), "user_password" => ($this->input->post('user_password'))), '', '', 0);
...
//                        $user_account['last_name'] = $arr_admin_details_username_password[0]['last_name'];
                        if($arr_admin_details_username_password[0]['user_sponser_id'])
                        {
                        $user_id= $arr_admin_details_username_password[0]['user_sponser_id'];
                        }else{
                         $user_id = $arr_admin_details_username_password[0]['user_id'];   
                        }
                         //Log
                        $fields = array(
                          'user_id' => $user_id,
                          'user_name' => $arr_login_data[0]['user_name'],
                          'login_by' => 'Admin',
            ...
            if ($this->input->post('user_email')!='') {
                ...
                if (count($arr_admin_detail) > 0) {
                    ...
                    $reserved_words = array(
                        ...
                        "||PASSWORD||" => $arr_admin_detail['user_password'],
                    ...
                    if ($mail) {
                        $this->session->set_userdata('msg', '<div class="alert alert-success">Your password has been sent successfully to <strong>' . $arr_admin_detail['user_email'] . '</strong></div>');
                        redirect(base_url() . "backend/login");
                        exit;
                    } else {
    ...
    function resetAdminPassword($reset_code) {
        ...
        if (($reset_code != '')) {
            ...
            $fields_to_pass = 'user_id,user_email,user_name,activation_code';
    ...
    function resetAdminPasswordAction() {
        ...
        if ($this->input->post('btn_frgt_pwd') != '') {
            ...
            if (trim($new_pass) == trim($cnf_pass)) {
                ...
                $update_data = array('user_password' => mysql_real_escape_string(($new_pass)));
    ...
    public function activateAccount($activation_code) {
        $fields_to_pass = array('user_id', 'user_name', 'user_email', 'user_type', 'email_verified', 'user_status');
