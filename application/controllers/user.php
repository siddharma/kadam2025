...
//        echo "<pre>";print_r($data['arr_user_list']);die;
    ...
    public function listUser() {
        ...
        $this->load->model('user_model');
        $data['title'] = "Manage User";
        $data['arr_user_list'] = $this->user_model->getRecentUser('');
//        echo "<pre>";print_r($data['arr_user_list']);die;
    ...
    public function changeMemberStatus() {
        if ($this->input->post('user_id') != "") {
//        echo "ss<pre>";print_r($this->input->post());echo "</pre>";die;
            /* updating the user status. */
            ...
            $arr_to_update = array(
              ...
              $condition_array = array('user_id' => ($this->input->post('user_id')));
              $this->common_model->updateRow('mst_users', $arr_to_update, $condition_array);
              
         if($this->input->post('user_status')=='Yes'){
            ...
            /* updating the global setttings parameter value into database */
             ...
             $arr_user_detail = $this->common_model->getRecords('mst_users', '*', array("user_id" => $this->input->post('user_id')));
             ...
             $login_link = '<a href="' . base_url() . 'signin">Click here</a>';
                ...
                $reserved_words = array(
                    "||USER_NAME||" => $arr_user_detail['full_name'],
                    "||USER_ID||" => $arr_user_detail['user_sponser_id'],
                    "||USER_EMAIL||" => $arr_user_detail['user_email'],
                    "||PASSWORD||" => $six_digit_random_number,
                    "||LOGIN_LINK||" => $login_link,
                    "||SITE_TITLE||" => mysql_real_escape_string($data['global']['site_title']),
                );
                $template_title = 'activate-user-email';
                $arr_emailtemplate_data = $this->common_model->getEmailTemplateInfo($template_title, '17', $reserved_words);
                $recipeinets = mysql_real_escape_string($arr_user_detail['user_email']);
...
//                        "user_name" => mysql_real_escape_string($this->input->post('user_name')),
                        ...
                        "email_verified" => mysql_real_escape_string($this->input->post('email_verified')),
                        "mobile_no" => mysql_real_escape_string($this->input->post('mobile_no')),
...
//                }
                ...
                redirect(base_url() . "backend/user/mainlist");
            }
        }
        $arr_privileges = array();
        /* getting all privileges */
        ...
        $data['arr_roles'] = $this->common_model->getRecords("mst_role");
//        $data['arr_cat'] = $this->common_model->getRecords("mst_category");
    ...
    public function userProfile($user_id) {
        ...
        $data['arr_user_detail'] = $this->user_model->getUserMembersDetails($user_id);
         //Get data for users updated form members
       
//               echo $user_id."<pre>ss";print_r($data['arr_user_detail']);die;
        /* single row fix */
    ...
    public function myaddition(){
            ...
            if ($first!='' && $second!='') {
                    ...
                    $arr_to_return = array('msg' => 'success', 'error_code' => '0', 'Addition is ' => $addition);
                    $response_arr = array('response' => $arr_to_return);

            } else {
                $arr_to_return = array('msg' => 'no input data', 'error_code' => '2');
                $response_arr = array('response' => $arr_to_return);
            }
